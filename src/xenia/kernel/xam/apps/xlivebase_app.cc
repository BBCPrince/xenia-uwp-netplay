...
X_HRESULT XLiveBaseApp::XPresenceGetState(uint32_t buffer_ptr,
  ...
  uint64_t xuid = xe::load_and_swap<uint64_t>(memory->TranslateVirtual(
      static_cast<uint32_t>(entry->xuid.argument_value_ptr)));
  const uint32_t state_flags_address =
      static_cast<uint32_t>(entry->state_flags_ptr.argument_value_ptr);
  const uint32_t session_id_address =
      static_cast<uint32_t>(entry->session_id_ptr.argument_value_ptr);

  if (!state_flags_address || !session_id_address) {
    return X_E_INVALIDARG;
  }

  auto state_flags_ptr =
      memory->TranslateVirtual<xe::be<uint32_t>*>(state_flags_address);
  auto session_id_ptr = memory->TranslateVirtual<XNKID*>(session_id_address);

  uint32_t state_flags = X_ONLINE_FRIENDSTATE_FLAG_NONE;
  XNKID session_id = {};

  UserProfile* local_profile =
      xuid ? kernel_state()->xam_state()->GetUserProfileAny(xuid)
           : kernel_state()->xam_state()->GetUserProfile(uint32_t(0));

  if (local_profile &&
      local_profile->signin_state() == X_USER_SIGNIN_STATE::SignedInToLive) {
    state_flags = X_ONLINE_FRIENDSTATE_FLAG_ONLINE |
                  X_ONLINE_FRIENDSTATE_FLAG_PLAYING |
                  X_ONLINE_FRIENDSTATE_ENUM_ONLINE;
  } else if (xuid) {
    for (uint32_t i = 0; i < XUserMaxUserCount; ++i) {
      ...
      UpdatePresenceXUIDs(std::vector<uint64_t>{xuid}, i);

      X_ONLINE_FRIEND friend_presence = {};
      if (profile->GetFriendFromXUID(xuid, &friend_presence)) {
        state_flags = friend_presence.state;
        session_id = friend_presence.session_id;
        break;
      }

      X_ONLINE_PRESENCE subscription_presence = {};
      if (profile->GetSubscriptionFromXUID(xuid, &subscription_presence)) {
        state_flags = subscription_presence.state;
        session_id = subscription_presence.session_id;
        break;
      }
    }
  }

  *state_flags_ptr = state_flags;
